...
namespace CodeContractor.Refactorings
{
    ...
    public sealed class AddRequiresRefactoringProvider : CodeRefactoringProvider
    {
        public override async Task ComputeRefactoringsAsync(CodeRefactoringContext context)
        {
            ...
            var refactoring = await AddNotNullRequiresRefactoring.Create(node, await context.Document.GetSemanticModelAsync());
            bool isAwailable = await refactoring.IsAvailableAsync(context.CancellationToken);

            if (isAwailable)
            {
                // For any type declaration node, create a code action to reverse the identifier text.
                var action = CodeAction.Create("Add not-null Contract.Requires", c => refactoring.ApplyRefactoringAsync(context.Document, context.CancellationToken));
